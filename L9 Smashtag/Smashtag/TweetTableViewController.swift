    ...
    var searchText: String? {
        didSet {
            ...
            if let text = searchText {
                RecentSearchHistory.addSearch(text)
            }
        }
    }
...
//        cell.detailTextLabel?.text = tweet.user.name
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let identifier = segue.identifier {
            if identifier == "TweetMentionsSegue",
                let tweetTableViewCell = sender as? TweetTableViewCell,
                let mentionsTableView = segue.destination as? MentionsTableViewController {
                mentionsTableView.tweet = tweetTableViewCell.tweet
            }
        }
    }
}
